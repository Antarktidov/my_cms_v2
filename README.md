Простая учебная CMS, написанная на PHP, с расширениями и скинами.

CMS учебная, поэтому здесь нет многих функций, которые должны быть в настоящей CMS. Можно создать и читать блоги, а также комментировать их (если включено расширение comments). В CMS присутствует защита от SQL-инъекций, CSRF-атак, а спецсимволы HTML экранируются при выводе.
...
Для запуска вам понадобится PHP (у автора PHP 8.5), MySQL или MariaDB и веб-сервер Apache (автор использует готовую сборку — WAMP). Папка с проектом должна находиться внутри соответствующей папки вашего сервера (для XAMPP это htdocs, для WAMP — www).
...
> [!CAUTION]
> Если ваш файл localSetting.php содержит конфиденциальную информацию, он должен быть добавлен в .gitignore.

# Скины

CMS содержит три скина: example, darky и other-side. Они лежат в папке skins. Чтобы изменить активный скин, измените переменную $my_cms_skin в localSetting.php. В примере ниже активный скин — "other-side".
...
Придумайте название собственного скина и создайте папку с ним в папке skins. Структура вашей папки со скином должна выглядеть так:
```bash
your-skin-name
│───css
|   └── skin.css
...
skin.css содержит ваш css, а skin.js — js.

В файле skin.php находится, собственно, ваш скин. Структура файла должна быть такой:
```php
<?php
...
* Расширения, добавляющие страницы: смотри пример contact_page в папке extensions.
...
<путь к CMS>/contact — форма обратной связи (обеспечивается расширением contact_page)
